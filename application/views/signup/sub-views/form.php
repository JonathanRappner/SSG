...
<form class="ssg_form" action="<?php echo base_url('signup/submit_signup');?>" method="post">
	...
	<div class="custom-control custom-radio">
		<input <?php echo (isset($signup) && $signup->attendance_id == 1) || !isset($signup) ? 'checked' : null;?> type="radio" id="attendance_yes" name="attendance" value="1" class="custom-control-input">
		<label class="custom-control-label text-signed" for="attendance_yes">Ja</label>
	</div>
	<div class="custom-control custom-radio">
		<input <?php echo isset($signup) && $signup->attendance_id == 2 ? 'checked' : null;?> type="radio" id="attendance_jip" name="attendance" value="2" class="custom-control-input">
		<label class="custom-control-label text-jip" for="attendance_jip"><abbr title="Join in progress">JIP</abbr></label>
	</div>
	<div class="custom-control custom-radio">
		<input <?php echo isset($signup) && $signup->attendance_id == 3 ? 'checked' : null;?> type="radio" id="attendance_qip" name="attendance" value="3" class="custom-control-input">
		<label class="custom-control-label text-qip" for="attendance_qip"><abbr title="Quit in progress">QIP</abbr></label>
	</div>
	<div class="custom-control custom-radio">
		<input <?php echo isset($signup) && $signup->attendance_id == 4 ? 'checked' : null;?> type="radio" id="attendance_noshow" name="attendance" value="4" class="custom-control-input">
	...
	<div class="form-group">
		...
		<textarea class="form-control" id="input_message" name="message" rows="3" placeholder="Övrig information angående din anmälan."><?php echo $signup != null ? $signup->message : null;?></textarea>
	</div>
	...
	<div class="mt-1">
		...
		<button type="submit" class="btn btn-success">
			<?php echo $this->member_not_signed ? 'Skapa anmälan <i class="fas fa-arrow-circle-right"></i>' : 'Spara ändringar <i class="fas fa-save"></i>'; ?>
		</button>
